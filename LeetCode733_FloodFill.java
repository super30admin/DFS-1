...
/*TC- O(mn)+O(mn) = O(mn) 
 * sc - O(v+E) - when we create adjacent list, if not then it's Max of (V,E)!!
 * here, we just create queue for nodes(v) - so it's Max of (v, E) ; v = m*n; O(mn);
*/
...
class Solution {
    ...
    public int[][] floodFill(int[][] image, int sr, int sc, int color) {
        ...
        if(image.length ==0 || image == null || image[sr][sc] == color)  return image;
        ...
        //store the old color of the cell to check if it neighbor has the same color as cell itself
